...
const _ws = `<script>let ws;let _b=1000;let _r=false;let _c=[];function _ws(){ws=new WebSocket("",_r?"reconnect":undefined);_r=true;if(_c.length>0){_c.forEach(e=>ws.addEventListener(...e))}ws.onopen=function(){_b=1000};ws.onmessage=function(e){if(e.data=="reload"){window.location.reload()}};ws.onclose=function(){setTimeout(()=>{_b*=2;_ws()},_b)}}_ws();const b=ws.addEventListener;ws.addEventListener=function(x,y,z){_c.push([x,y,z]);b.call(ws,x,y,z)}</script>`;
