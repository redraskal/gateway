...
const _ws = `<script>let ws;let _b=1000;let c=[];function _ws(){ws=new WebSocket("");if(c.length>0){c.forEach(e=>ws.addEventListener(...e))}ws.onopen=function(){_b=1000};ws.onclose=function(){setTimeout(()=>{_b*=2;_ws()},_b)}}_ws();const b=ws.addEventListener;ws.addEventListener=function(x,y,z){c.push([x,y,z]);b.call(ws,x,y,z)}</script>`;
